...
export const httpAgent = function() {
    return new SocksProxyAgent(`socks5h://${torConfig.ip}:${torConfig.port}`);
}

export const httpsAgent = function() {
    return new SocksProxyAgent(`socks5h://${torConfig.ip}:${torConfig.port}`);
}

export function torSetup({ ip = 'localhost', port = 9050, path = '', controlPort = '9051', controlPassword = 'giraffe' }) {
