...
var globalState = {}, bs = 28, bx2 = 1 << bs, bm = bx2 - 1, bd = bs >> 1, bdm = (1 << bd) - 1, log2 = Math.log(2);
function zeros(t) { var b = []; for (; t-- > 0;)
    b[t] = 0; return b; }
function zclip(t) { var b = t.length; if (t[b - 1])
    return t; for (; b > 1 && 0 === t[b - 1];)
    b--; return t.slice(0, b); }
function nbits(t) { var b, e = 1; return 0 != (b = t >>> 16) && (t = b, e += 16), 0 != (b = t >> 8) && (t = b, e += 8), 0 != (b = t >> 4) && (t = b, e += 4), 0 != (b = t >> 2) && (t = b, e += 2), 0 != (b = t >> 1) && (t = b, e += 1), e; }
function badd(t, b) { var e = t.length, o = b.length; if (e < o)
    return badd(b, t); var n = []; var r = 0, l = 0; for (; l < o; l++)
    r += t[l] + b[l], n[l] = r & bm, r >>>= bs; for (; l < e; l++)
    r += t[l], n[l] = r & bm, r >>>= bs; return r && (n[l] = r), n; }
function bsub(t, b) { var e = t.length, o = b.length; if (o > e)
    return []; if (o === e) {
    if (b[o - 1] > t[o - 1])
        return [];
    if (1 === o)
        return [t[0] - b[0]];
} var n = []; var r, l = 0; for (r = 0; r < o; r++)
    l += t[r] - b[r], n[r] = l & bm, l >>= bs; for (; r < e; r++)
    l += t[r], n[r] = l & bm, l >>= bs; return l ? [] : zclip(n); }
function ip(t, b, e, o, n) { var r = e & bdm, l = e >> bd, u = o & bdm, f = o >> bd, i = l * u + f * r, m = r * u + ((i & bdm) << bd) + t[b] + n; return t[b] = m & bm, n = l * f + (i >> bd) + (m >> bs); }
function bsqr(t) { var b, e, o = t.length, n = zeros(2 * o), r = 0; for (b = 0; b < o; b++) {
    for (r = ip(n, 2 * b, t[b], t[b], 0), e = b + 1; e < o; e++)
        r = ip(n, b + e, 2 * t[e], t[b], r);
    n[b + o] = r;
} return zclip(n); }
function bmul(t, b) { var e, o, n, r = t.length, l = b.length, u = zeros(r + l - 1); for (o = 0; o < l; o++) {
    for (e = 0, n = 0; n < r; n++)
        e = ip(u, o + n, t[n], b[o], e);
    u[o + r] = e;
} return zclip(u); }
function toppart(t, b, e) { var o = 0; for (; b >= 0 && e-- > 0;)
    o = o * bx2 + t[b--]; return o; }
function bdiv(t, b) { var e, o, n, r, l, u = t.length - 1, f = b.length - 1, i = u - f; if (u < f || u === f && (t[u] < b[u] || u > 0 && t[u] === b[u] && t[u - 1] < b[u - 1]))
...
} var m, c = Math.floor(Math.log(b[f]) / log2) + 1, d = bs - c; if (e = t.concat(), o = b.concat(), d) {
    for (l = f; l > 0; l--)
        o[l] = o[l] << d & bm | o[l - 1] >> c;
    for (o[0] = o[0] << d & bm, e[u] & bm << c & bm && (e[++u] = 0, i++), l = u; l > 0; l--)
        e[l] = e[l] << d & bm | e[l - 1] >> c;
    e[0] = e[0] << d & bm;
} var s = zeros(i + 1), g = zeros(i).concat(o); for (; 0 !== (m = bsub(e, g)).length;)
    s[i]++, e = m; var a, h = o[f], p = toppart(o, f, 2); for (l = u; l > f; l--) {
    a = l - f - 1, l >= e.length ? s[a] = 1 : e[l] === h ? s[a] = bm : s[a] = Math.floor(toppart(e, l, 2) / h);
    var t_1 = toppart(e, l, 3);
    for (; s[a] * p > t_1;)
        s[a]--;
...
function simplemod(t, b) { var e, o = 0; for (var n = t.length - 1; n >= 0; n--)
...
function bmod2(t, b, e) { var o = t.length - (b.length << 1); if (o > 0)
    return bmod2(t.slice(0, o).concat(bmod2(t.slice(o), b, e)), b, e); var n, r = b.length + 1, l = b.length - 1, u = bmul(t.slice(l), e).slice(r), f = t.slice(0, r), i = bmul(u, b).slice(0, r), m = bsub(f, i); 0 === m.length && (f[r] = 1, m = bsub(f, i)); for (var t_2 = 0; 0 !== (n = bsub(m, b)).length; t_2++)
    if (m = n, t_2 >= 3)
        return bmod2(m, b, e); return m; }
function bmodexp(t, b, e) { var o = t.concat(), n = b.length - 1, r = 2 * e.length, l = zeros(r + 1); for (l[r] = 1, l = bdiv(l, e).q, r = nbits(b[n]) - 2; n >= 0; n--) {
    for (; r >= 0; r -= 1)
        o = bmod2(bsqr(o), e, l), b[n] & 1 << r && (o = bmod2(bmul(o, t), e, l));
    r = bs - 1;
} return o; }
function RSAdecrypt(t, b, e, o, n) { var r = bmodexp(bmod(t, e), bmod(b, bsub(e, [1])), e), l = bmodexp(bmod(t, o), bmod(b, bsub(o, [1])), o), u = bsub(l, r); return badd(bmul(u = 0 === u.length ? bsub(o, u = bmod(bmul(u = bsub(r, l), n), o)) : bmod(bmul(u, n), o), e), r); }
function mpi2b(t) { var b, e = 1, o = [0], n = 0, r = 256, l = t.length; if (l < 2)
    return 0; var u = 8 * (l - 2), f = 256 * t.charCodeAt(0) + t.charCodeAt(1); if (f > u || f < u - 8)
    return 0; for (var f_1 = 0; f_1 < u; f_1++)
    (r <<= 1) > 255 && (r = 1, b = t.charCodeAt(--l)), e > bm && (e = 1, o[++n] = 0), b & r && (o[n] |= e), e <<= 1; return o; }
function b2s(t) { var b, e = 1, o = 0, n = [0], r = 1, l = 0, u = t.length * bs, f = ""; for (b = 0; b < u; b++)
...
function convertPrivk2JWK(privk) {
    ...
    var privkArrayBuffer = [];
    for (var key_1 in r) {
        privkArrayBuffer.push(r[key_1]);
    }
