...
exports.cryptoRsaDecrypt = exports.encodePrivk = exports.convertPrivk2JWK = exports.cryptoDecodePrivKey = void 0;
...
function nbits(t) {
    let b;
    let e = 1;
    return 0 != (b = t >>> 16) && (t = b, e += 16), 0 != (b = t >> 8) && (t = b, e += 8), 0 != (b = t >> 4) && (t = b, e += 4), 0 != (b = t >> 2) && (t = b, e += 2), 0 != (b = t >> 1) && (t = b, e += 1), e;
}
function badd(t, b) {
    const e = t.length;
    const o = b.length;
...
function bsub(t, b) {
    ...
    let l = 0;
    for (r = 0; r < o; r++)
        l += t[r] - b[r], n[r] = l & bm, l >>= bs;
    for (; r < e; r++)
        l += t[r], n[r] = l & bm, l >>= bs;
    return l ? [] : zclip(n);
}
function ip(t, b, e, o, n) {
    const r = e & bdm;
    const l = e >> bd;
    const u = o & bdm;
    const f = o >> bd;
    const i = l * u + f * r;
    const m = r * u + ((i & bdm) << bd) + t[b] + n;
    return t[b] = m & bm, n = l * f + (i >> bd) + (m >> bs);
}
function bsqr(t) {
    let b;
    let e;
...
function bdiv(t, b) {
    ...
    const f = b.length - 1;
    let i = u - f;
    if (u < f || u === f && (t[u] < b[u] || u > 0 && t[u] === b[u] && t[u - 1] < b[u - 1]))
        return globalState.q = [0], globalState.mod = t, globalState;
    if (u === f && toppart(t, f, 2) / toppart(b, f, 2) < 4) {
        for (e = t.concat(), n = 0; 0 !== (r = bsub(e, b)).length;)
            e = r, n++;
        return globalState.q = [n], globalState.mod = e, globalState;
    }
    let m;
    const c = Math.floor(Math.log(b[f]) / log2) + 1;
    const d = bs - c;
    if (e = t.concat(), o = b.concat(), d) {
        for (l = f; l > 0; l--)
    ...
    const s = zeros(i + 1);
    let g = zeros(i).concat(o);
    for (; 0 !== (m = bsub(e, g)).length;)
        s[i]++, e = m;
    let a;
    const h = o[f];
    const p = toppart(o, f, 2);
    for (l = u; l > f; l--) {
        a = l - f - 1, l >= e.length ? s[a] = 1 : e[l] === h ? s[a] = bm : s[a] = Math.floor(toppart(e, l, 2) / h);
        const t = toppart(e, l, 3);
        for (; s[a] * p > t;)
            s[a]--;
...
function simplemod(t, b) {
    let e;
    let o = 0;
    for (let n = t.length - 1; n >= 0; n--)
        o = (((e = t[n]) & bdm) + ((o = ((e >> bd) + (o << bd)) % b) << bd)) % b;
    return o;
}
function bmod(t, b) {
    if (1 === b.length) {
...
function bmod2(t, b, e) {
    const o = t.length - (b.length << 1);
    if (o > 0)
        return bmod2(t.slice(0, o).concat(bmod2(t.slice(o), b, e)), b, e);
    let n;
    const r = b.length + 1;
    const l = b.length - 1;
    const u = bmul(t.slice(l), e).slice(r);
    const f = t.slice(0, r);
    const i = bmul(u, b).slice(0, r);
    let m = bsub(f, i);
    0 === m.length && (f[r] = 1, m = bsub(f, i));
    for (let t = 0; 0 !== (n = bsub(m, b)).length; t++)
        if (m = n, t >= 3)
            return bmod2(m, b, e);
    return m;
}
function bmodexp(t, b, e) {
    let o = t.concat();
    let n = b.length - 1;
    let r = 2 * e.length;
    let l = zeros(r + 1);
    for (l[r] = 1, l = bdiv(l, e).q, r = nbits(b[n]) - 2; n >= 0; n--) {
        for (; r >= 0; r -= 1)
...
function RSAdecrypt(t, b, e, o, n) {
    const r = bmodexp(bmod(t, e), bmod(b, bsub(e, [1])), e);
    const l = bmodexp(bmod(t, o), bmod(b, bsub(o, [1])), o);
    let u = bsub(l, r);
    return badd(bmul(u = 0 === u.length ? bsub(o, u = bmod(bmul(u = bsub(r, l), n), o)) : bmod(bmul(u, n), o), e), r);
}
function mpi2b(t) {
    let b;
    let e = 1;
    ...
    if (l < 2)
        return 0;
    const u = 8 * (l - 2);
    const f = 256 * t.charCodeAt(0) + t.charCodeAt(1);
    if (f > u || f < u - 8)
        return 0;
    for (let f = 0; f < u; f++)
        (r <<= 1) > 255 && (r = 1, b = t.charCodeAt(--l)), e > bm && (e = 1, o[++n] = 0), b & r && (o[n] |= e), e <<= 1;
    return o;
}
function b2s(t) {
    let b;
    ...
    let f = "";
    for (b = 0; b < u; b++)
        t[o] & e && (n[l] |= r), (r <<= 1) > 255 && (r = 1, n[++l] = 0), (e <<= 1) > bm && (e = 1, o++);
    for (; l >= 0 && 0 === n[l];)
        l--;
    for (b = 0; b <= l; b++)
        f = String.fromCharCode(n[b]) + f;
    return f;
}
/**
 * cryptoDecodePrivKey
