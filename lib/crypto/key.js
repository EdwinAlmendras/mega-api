...
class prepare {
     ...
     */
    static v2(password, salt) {
        if (!(salt instanceof Buffer))
            salt = Buffer.from(salt, 'base64');
        console.log("salt", salt);
        const iterations = 100000;
        const digest = 'sha512';
        const deriveKey = crypto_1.default.pbkdf2Sync(password, salt, iterations, 32, digest);
        return {
...
function deriveKeys(password, masterKey) {
    ...
    const { passwordKey, userHash } = exports.key.prepare.v2(Buffer.from(password, "utf8"), salt);
