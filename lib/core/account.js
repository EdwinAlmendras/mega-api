...
class MegaAccount extends events_1.EventEmitter {
    ...
    login({ email, password, fetch }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const sessionIdBuffer = crypto_1.cryptoRsaDecrypt(csid, RSA_PRIVK).slice(0, RSA_PRIVK_LENGTH);
            this.client.state.SESSION_ID = crypto_1.base64.encrypt(sessionIdBuffer);
            try {
                yield this.data();
                if (fetch)
    ...
    _loginGetHashAndPasswordKey(passwordBytes, email) {
        return __awaiter(this, void 0, void 0, function* () {
            const { v: version, s: salt } = yield this.client.api.request({ a: "us0", user: email });
            if (version === 1) {
                let credentials = crypto_1.key.prepare.v1(passwordBytes, email);
                return Promise.resolve(credentials);
            }
            else if (version === 2) {
                let credentials = crypto_1.key.prepare.v2(passwordBytes, salt);
                return Promise.resolve(credentials);
            }
            else {
